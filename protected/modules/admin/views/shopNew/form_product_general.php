...
if($model->category instanceof Category) {
	if($ancestors = $model->category->ancestors()->findAll()) { 
	  foreach($ancestors as $i=>$cat){
    	$breadcrumbs[$cat->title] = array('shopNew/category', 'id'=>$cat->id);
	  }
	}

	$breadcrumbs[$model->category->title] = array('shopNew/category', 'id'=>$model->category->id);
}
...
<div class="form">
    ...
    <div class="row buttons">
      ...
      <?php if (!$model->isNewRecord): ?>
        <div class="right">
          <a href="<?php echo $this->createUrl('shopNew/productDelete', array('id'=>$model->id)); ?>"onclick="return confirm('Вы действительно хотите удалить товар?')" class="btn btn-danger">Удалить товар</a>
        </div>
        <div class="right">
          <a href="<?php echo $this->createUrl('shopNew/productclone', array('id'=>$model->id)); ?>" class="btn btn-info">Клонировать товар</a>
        </div>
      <?php endif; ?>
      <div class="clr"></div>
    </div>
