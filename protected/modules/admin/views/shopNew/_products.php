  ...
  <ul id="product-list" class="product-list row">
    ...
    <li id="item_<?php echo $product->id ?>" data-sort-id="<?= $product->id; ?>" class="col-xs-3<? if(!$isIndexPage && ($product->category_id <> $category->id)) echo ' bg-warning'; ?><? if($product->hidden) echo ' mark-hidden'; ?>">
      <div class="product thumbnail">
        ...
        <div class="img">
        	<?= CHtml::link($product->img(195, 195), ['shopNew/productUpdate', 'id'=>$product->id]); ?>
        </div>
        <div class="caption">
          <p class="title" title="<?php echo $product->title ?>"><?php echo Chtml::link($product->title, array('shopNew/productUpdate', 'id'=>$product->id)); ?></p>
          <?= CHtml::link('<div class="btn btn-default btn-sm"><span class="price">'.$product->price.'</span> руб.</div>', ['shopNew/productUpdate', 'id'=>$product->id]); ?>
          <?= CHtml::link('Удалить', ['shopNew/productDelete', 'id'=>$product->id], [
