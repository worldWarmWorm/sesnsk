...
class ProductNew extends \common\components\base\ActiveRecord
{
	protected $moreImg;
    /**
     * (non-PHPdoc)
...
//         	'filesBehavior'=>'\file\behaviors\FilesBehavior',
    ...
    public function relations()
    {
        return $this->getRelations(array(
            ...
            'category'=>array(self::BELONGS_TO, 'CategoryNew', 'category_id'),
    ...
    { 
        return $this->getScopes([
        	...
        	'cardColumns'=>[
        		'select'=>'`t`.`id`, `t`.`category_id`, title, code, price, sale, new, hit, link_title, notexist, old_price, `main_image`, `main_image_alt`, `main_image_enable`'
        	],
    		...
    		if(count($attr_filter)>0){
    			...
    			if(!empty($values)) {
    				...
    				if(!empty($eavValues)) {
    					...
    					$query.=' GROUP BY id_product HAVING count(id_product)='.count($eavValues);
    ...
    public function byCategory($id, $depthDescendants=false, $withRelated=false)
    {
    	...
    	$ids=[];
       	if($depthDescendants) {
	        $category=CategoryNew::model()->findByPk($id);
	        if($category) {
...
/*        $orderCriteria=['scopes'=>['hitOnTop']];
        if($categoryId) {
        	$orderCriteria['scopes']['scopeSort']=['shop_category_new', $categoryId];
...
*/      
        ...
        $modelProduct=new ProductNew();
        $criteria->mergeWith($modelProduct->scopeSort('shop_category_new', $categoryId)->getDbCriteria());
        $modelProduct=new ProductNew();
    ...
    protected function beforeValidate()
    {
        ...
        $this->old_price=str_replace(',','.', str_replace(' ', '', $this->old_price));
