...
class ShopNewController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider=ProductNew::model()
        	->onShopIndex()
        	->visibled()
        	->cardColumns()
        	->scopeSort('shop_category_new')
    ...
    {
        $category = $this->loadModel('CategoryNew', $id);
        
        $model = new ProductNew('search');
        ...
        if(\Yii::app()->request->isAjaxRequest) {
	        ...
	        $this->breadcrumbs->add($this->getHomeTitle(), '/shopNew');
	        $this->breadcrumbs->addByNestedSet($category, '/shopNew/category');
    ...
    {
        $product = ProductNew::model()->visibled()->findByPk($id);
        ...
        if($categoryId=\Yii::app()->request->getParam('category_id')) {
        	$category=CategoryNew::model()->findByPk($categoryId);
        }
        if(!$category) {
        	$category=$product->category;
        }
        $this->breadcrumbs->addByNestedSet($category, '/shopNew/category');
        $this->breadcrumbs->add($category->title, array('/shopNew/category', 'id'=>$category->id));
    ...
    {
    	$params=ProductNew::model()->getFilterRequestParams('filter');
    	$dataProvider=ProductNew::model()->visibled()->filter('filter', A::m([
    		['Product', 'filterDefaultHandler'],
    	], ProductNew::model()->rangeofBehavior->getFilterHandler()))->eav()->getDataProvider([
    		'pagination'=>[
